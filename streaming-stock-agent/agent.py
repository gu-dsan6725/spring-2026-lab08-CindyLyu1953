...
def _get_stock_history(ticker: str, days: int = 30) -> Dict[str, Any]:
...
def _format_market_cap(market_cap: Any) -> Optional[str]:
    """Format market cap number to human-readable string (e.g., 2.8T, 500B)."""
...
def _compare_stocks(symbol1: str, symbol2: str) -> Dict[str, Any]:
    ...
    """
    try:
        price1 = _get_stock_price(symbol1)
        price2 = _get_stock_price(symbol2)
        info1 = _get_company_info(symbol1)
        info2 = _get_company_info(symbol2)

        if "error" in price1:
            return {"error": price1["error"], "symbol1": symbol1.upper()}
        if "error" in price2:
            return {"error": price2["error"], "symbol2": symbol2.upper()}

        def build_stock_data(price: Dict, info: Dict, symbol: str) -> Dict[str, Any]:
            market_cap = info.get("market_cap") if "error" not in info else None
            return {
                "symbol": symbol.upper(),
                "current_price": price.get("current_price"),
                "company_name": price.get("name") or info.get("name") or symbol.upper(),
        ...
        return {
            "comparison": {
                ...
                "stock1": build_stock_data(price1, info1, symbol1),
                "stock2": build_stock_data(price2, info2, symbol2),
            }
        }
    except Exception as e:
...
STOCK_TOOLS = [
    ...
    {
        "name": "compare_stocks",
        "description": "Compare two stocks side-by-side with current price, company name, and market cap. Use this when the user asks to compare two stocks, e.g. 'Compare AAPL and MSFT', 'Which is better Tesla or Ford?', or 'Compare Apple with Microsoft'.",
        "parameters": {
            "type": "object",
            "properties": {
                "symbol1": {
                    "type": "string",
                    "description": "First stock symbol to compare (e.g., AAPL, TSLA)",
                },
                "symbol2": {
                    "type": "string",
                    "description": "Second stock symbol to compare (e.g., MSFT, F)",
                },
            },
...
def execute_tool_call(tool_name: str, parameters: Dict[str, Any]) -> str:
