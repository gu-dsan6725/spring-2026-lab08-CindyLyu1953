...
def _detect_subscriptions(
    ...
    for txn in bank_transactions + credit_card_transactions:
        ...
        if amount >= 0:
            continue
        service = txn.get("description") or txn.get("merchant", "Unknown")
        amount_abs = abs(amount)
        key = f"{service}_{amount_abs}"
...
async def _fetch_financial_data(
    ...
    async with Client("http://127.0.0.1:5001/mcp") as bank_client:
        result = await bank_client.call_tool("get_bank_transactions", args)
        bank_data = result.data

    async with Client("http://127.0.0.1:5002/mcp") as cc_client:
        result = await cc_client.call_tool("get_credit_card_transactions", args)
        credit_card_data = result.data
